...
class Hpp2Controller extends BaseHppController
...
public function store(Request $request)
{
    try {
        ...
        $data['source_form'] = $data['source_form'] ?? 'createhpp2';
        ...
        \Log::error('Gagal menyimpan HPP2 (store): '.$e->getMessage(), ['trace' => $e->getTraceAsString()]);
...
private function issueFirstToken(Hpp1 $hpp): void
{
    ...
    public function edit($notification_number)
    {
        try {
            ...
            Log::error('Gagal membuka halaman edit HPP2: '.$e->getMessage());
...
public function update(Request $request, $notification_number)
{
    ...
    public function downloadPDF($notification_number)
    {
        try {
            ...
            $pdf = PDF::loadView('admin.inputhpp.hpppdf2', $data)
                ->setPaper('a4', 'landscape', 'none');

            return $pdf->stream('HPP2.pdf');
        } catch (\Throwable $e) {
    ...
    public function destroy($notification_number)
    {
        try {
            ...
            Log::error('Gagal menghapus HPP2: '.$e->getMessage());
            return redirect()
                ->route('admin.inputhpp.index')
