...
class Hpp4Controller extends BaseHppController
{public function index()
{
    ...
    public function create()
    {
        try {
            ...
            $source_form = 'createhpp4';
            $currentOA   = $this->getCurrentOA();

            return view('admin.inputhpp.createhpp4', compact('notifications', 'source_form', 'currentOA'));
        } catch (\Throwable $e) {
            Log::error('Gagal membuka form create HPP4: '.$e->getMessage());
...
public function store(Request $request)
{
    try {
        ...
        $data['source_form'] = $data['source_form'] ?? 'createhpp4';
        ...
        \Log::error('Gagal menyimpan HPP3 (store): '.$e->getMessage(), ['trace' => $e->getTraceAsString()]);
...
private function issueFirstToken(Hpp1 $hpp): void
{
    ...
    public function edit($notification_number)
    {
        try {
            ...
            Log::error('Gagal membuka halaman edit HPP4: '.$e->getMessage());
...
public function update(Request $request, $notification_number)
{
    ...
    public function downloadPDF($notification_number)
    {
        try {
            ...
            $pdf = PDF::loadView('admin.inputhpp.hpppdf4', $data)
                ->setPaper('a4', 'landscape', 'none');

            return $pdf->stream('HPP4.pdf');
        } catch (\Throwable $e) {
    ...
    public function destroy($notification_number)
    {
        try {
            ...
            Log::error('Gagal menghapus HPP4: '.$e->getMessage());
            return redirect()
                ->route('admin.inputhpp.index')
