...
class Hpp6Controller extends BaseHppController
{public function index()
{
    ...
    public function create()
    {
        try {
            ...
            $source_form = 'createhpp6';
            $currentOA   = $this->getCurrentOA();

            return view('admin.inputhpp.createhpp6', compact('notifications', 'source_form', 'currentOA'));
        } catch (\Throwable $e) {
            Log::error('Gagal membuka form create HPP6: '.$e->getMessage());
...
public function store(Request $request)
{
    try {
        ...
        $data['source_form'] = $data['source_form'] ?? 'createhpp6';
        ...
        \Log::error('Gagal menyimpan HPP6 (store): '.$e->getMessage(), ['trace' => $e->getTraceAsString()]);
...
private function issueFirstToken(Hpp1 $hpp): void
{
    ...
    public function edit($notification_number)
    {
        try {
            ...
            Log::error('Gagal membuka halaman edit HPP6: '.$e->getMessage());
...
public function update(Request $request, $notification_number)
{
    try {
        ...
        \Log::error('Gagal memperbarui HPP6: '.$e->getMessage(), ['trace' => $e->getTraceAsString()]);
    ...
    public function downloadPDF($notification_number)
    {
        try {
            ...
            $pdf = PDF::loadView('admin.inputhpp.hpppdf6', $data)
                ->setPaper('a4', 'landscape', 'none');

            return $pdf->stream("HPP-{$hpp->notification_number}.pdf");
        } catch (\Throwable $e) {
            Log::error('Gagal membuat PDF HPP6: '.$e->getMessage());
    ...
    public function destroy($notification_number)
    {
        try {
            ...
            Log::error('Gagal menghapus HPP6: '.$e->getMessage());
            return redirect()
                ->route('admin.inputhpp.index')
