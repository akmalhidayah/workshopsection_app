<x-admin-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-800">Tambah Unit Kerja</h2>
    </x-slot>

    <div class="p-6 bg-white shadow rounded-lg max-w-lg mx-auto">
        <form action="{{ route('admin.unit_work.store') }}" method="POST">
            @csrf
            <div class="mb-4">
                <label class="block font-medium text-sm text-gray-700">Nama Unit Kerja</label>
                <input type="text" name="name" class="w-full border border-gray-300 rounded px-3 py-2 mt-1" value="{{ old('name') }}">
                @error('name')
