<x-approval>
  @php
    // Pilih route PDF berdasar source_form
    ...
    // Tentukan default note_target berdasar sign_type (server-side supaya pasti)
    $defaultNoteTarget = in_array(($sign_type ?? ''), ['manager','sm','gm','dir']) ? 'controlling' : 'requesting';
  @endphp
  ...
  <div class="max-w-6xl mx-auto py-5 px-3 md:px-0 space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
      <div class="space-y-0.5">
        <h2 class="text-base md:text-xl font-semibold text-slate-800">Approval HPP – {{ $signTypeLabel }}</h2>
        <p class="text-[11px] md:text-xs text-slate-500">Tinjau pratinjau dokumen, lalu tanda tangani di bawah.</p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div class="bg-white border border-slate-200 rounded-xl p-4 md:col-span-2">
        <dl class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-[12px] md:text-sm">
          <div>
            ...
            <dd class="font-semibold text-slate-800 break-all">{{ $hpp->notification_number }}</dd>
          </div>
          <div>
            <dt class="text-slate-500">Nilai Total</dt>
            <dd class="font-semibold text-slate-800">Rp {{ number_format((float)$hpp->total_amount, 2, ',', '.') }}</dd>
          </div>
          <div class="sm:col-span-2">
            <dt class="text-slate-500">Deskripsi</dt>
            <dd class="font-medium text-slate-800">{{ $hpp->description }}</dd>
          </div>
        </dl>
      </div>
      ...
      <div class="bg-white border border-slate-200 rounded-xl p-4 flex flex-col gap-2">
        ...
        <div class="flex flex-wrap gap-2">
          <a href="{{ $pdfUrl }}" target="_blank" class="px-3 py-1.5 text-xs rounded-lg bg-slate-900 text-white hover:bg-slate-800">
            Buka di Tab Baru
          </a>
          <a href="{{ $pdfUrl }}" download class="px-3 py-1.5 text-xs rounded-lg bg-slate-100 text-slate-800 hover:bg-slate-200">
            Download PDF
          </a>
        </div>
        <div class="text-[11px] text-slate-500">Tip: gunakan zoom di viewer (Ctrl +/–) bila perlu.</div>
      </div>
    </div>

    <!-- PDF Preview -->
    <div class="bg-white border border-slate-200 rounded-xl overflow-hidden">
      <div class="h-12 md:h-14 flex items-center justify-between px-3 md:px-4 border-b">
        <div class="text-sm md:text-[15px] font-semibold text-slate-800">Pratinjau PDF</div>
        <div class="text-[10px] md:text-[11px] text-slate-500">Jika PDF tidak tampil, klik “Buka di Tab Baru”.</div>
      </div>
      ...
      <div id="pdfWrap" class="relative">
        ...
        <!-- pakai iframe agar kompatibel & bisa param zoom -->
        <iframe
          id="pdfFrame"
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-4">
          ...
          <div class="border rounded-lg bg-slate-50 overflow-hidden">
            ...
            <div class="px-3 pb-3 flex flex-wrap gap-2">
              <button type="button" id="btnUndo" class="px-3 py-1.5 text-xs rounded-lg bg-slate-200 hover:bg-slate-300">Undo</button>
              <button type="button" id="btnClear" class="px-3 py-1.5 text-xs rounded-lg bg-slate-200 hover:bg-slate-300">Clear</button>
            </div>
          </div>
        ...
        <div class="bg-white border border-slate-200 rounded-xl p-4 space-y-4">
          <div>
            ...
            <p class="text-[11px] text-slate-500 mb-2">Akan disimpan ke
              <span class="font-medium" id="noteTargetLabel">{{ $defaultNoteTarget === 'controlling' ? 'Controlling Notes' : 'Requesting Notes' }}</span>.
            </p>
            <textarea name="note" rows="5" class="w-full border rounded-lg p-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200" placeholder="Tulis catatan singkat approval (opsional)…"></textarea>
          </div>

          <div>
            <label class="block text-sm font-semibold text-slate-800">Alasan Penolakan</label>
            <p class="text-[11px] text-slate-500 mb-2">Wajib diisi bila menekan <b>Tolak</b>.</p>
            <textarea name="reason" rows="4" class="w-full border rounded-lg p-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-200" placeholder="Tulis alasan penolakan (jika menolak)…"></textarea>
          </div>

          <div class="flex flex-col sm:flex-row gap-2">
            <button name="action" value="approve" class="flex-1 px-4 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-500">
              Setujui
            </button>
            <button name="action" value="reject" class="flex-1 px-4 py-2 rounded-lg bg-rose-600 text-white hover:bg-rose-500"
  ...
  <script>
    ...
    let drawing = false;
    const strokes = []; // stack untuk undo (array of paths)
    let current = [];   // path aktif

    // ⚙️ Parameter tampilan & export
    const VIEW_WIDTH   = 2;
    ...
    function start(p){ drawing = true; current = [p]; }
    function move(p){
      if(!drawing) return;
    ...
    document.getElementById('btnClear')?.addEventListener('click', () => { strokes.length=0; redraw(); });
    document.getElementById('btnUndo')?.addEventListener('click', () => { strokes.pop(); redraw(); });
