...
.sig-box > img{
  position: absolute;
  left: 50%;
  bottom: -15px;      /* lebih turun agar proporsional */
...
<table style="width: 100%; border: 1px solid black; border-collapse: collapse; font-size: 11px;">
    <tr>
       ...
       <!-- BAGIAN KANAN: FUNGSI PEMINTA (HPP KHUSUS: 2 kolom dinamis) -->
<td style="width: 18%; vertical-align: top; padding: 4px; border-left: 1px solid black;">
    <div style="border: 1px solid black; padding: 4px;">
        ...
        @php
            $hasMgrReq = !empty($SIG_REQ_MG) || !empty($hpp->managerSignatureRequestingUser);
            $hasSmReq  = !empty($SIG_REQ_SM) || !empty($hpp->seniorManagerSignatureRequestingUser);
            $hasGmReq  = !empty($SIG_REQ_GM) || !empty($hpp->generalManagerSignatureRequestingUser);

            // Tentukan 2 kolom peminta:
            // - kalau Manager ada -> Manager + (SM jika ada, else GM)
            // - kalau Manager tidak ada -> SM + GM
            if ($hasMgrReq) {
                $leftKey = 'mgr';
                $rightKey = $hasSmReq ? 'sm' : 'gm';
            } else {
                $leftKey = 'sm';
                $rightKey = 'gm';
            }

            $labelFor = fn($k) => $k === 'gm' ? 'GM Of' : ($k === 'sm' ? 'SM Of' : 'Mgr Of');
            $unitFor = fn($k) => $k === 'gm'
                ? ($hpp->generalManagerSignatureRequestingUser?->unit_work ?? 'N/A')
                : ($k === 'sm'
                    ? ($hpp->seniorManagerSignatureRequestingUser?->unit_work ?? 'N/A')
                    : ($hpp->managerSignatureRequestingUser?->unit_work ?? 'N/A'));
            $nameFor = fn($k) => $k === 'gm'
                ? ($hpp->generalManagerSignatureRequestingUser?->name ?? 'N/A')
                : ($k === 'sm'
                    ? ($hpp->seniorManagerSignatureRequestingUser?->name ?? 'N/A')
                    : ($hpp->managerSignatureRequestingUser?->name ?? 'N/A'));
            $sigFor = fn($k) => $k === 'gm' ? ($SIG_REQ_GM ?? null) : ($k === 'sm' ? ($SIG_REQ_SM ?? null) : ($SIG_REQ_MG ?? null));
            $dtFor  = fn($k) => $k === 'gm' ? ($DT_REQ_GM ?? '-') : ($k === 'sm' ? ($DT_REQ_SM ?? '-') : ($DT_REQ_MG ?? '-'));
        @endphp

        <table style="width: 100%; border-collapse: collapse; text-align: center;">
            <tr>
                <td style="width: 50%; border-right: 1px solid black; padding: 4px;">
                    <strong>{{ $labelFor($leftKey) }}</strong><br>
                    <span style="font-size: 10px;">{{ $unitFor($leftKey) }}</span>
                </td>
                <td style="width: 50%; padding: 4px;">
                    <strong>{{ $labelFor($rightKey) }}</strong><br>
                    <span style="font-size: 10px;">{{ $unitFor($rightKey) }}</span>
                </td>
            </tr>
            ...
            <tr>
                <td style="border-right: 1px solid black; padding: 4px; text-align: center; vertical-align: bottom;">
                    <div class="sig-box">
                        <div class="sig-date">{{ $dtFor($leftKey) }}</div>
                        @if($sigFor($leftKey))
                            <img src="{{ $sigFor($leftKey) }}" alt="TTD Fungsi Peminta" class="sig-img--lg">
                        @else
                ...
                <td style="padding:4px; text-align:center; vertical-align:bottom;">
                    <div class="sig-box">
                        <div class="sig-date">{{ $dtFor($rightKey) }}</div>
                        @if($sigFor($rightKey))
                            <img src="{{ $sigFor($rightKey) }}" alt="TTD Fungsi Peminta" class="sig-img--lg">
                        @else
                            <strong class="sig-fallback">TTD</strong>
                        @endif
                    </div>
...
<td style="width: 35%; border: 1px solid black; vertical-align: top; padding: 8px;">
  ...
  @endif
        ...
        <td class="px-2 py-2" style="width: 50%; border: 1px solid black;">
            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    <td colspan="2" class="px-2 py-2 text-center" style="border-bottom: 1px solid black; font-weight: bold; font-style: italic;">
                        FUNGSI PENGENDALI
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td class="px-2 py-4 text-center" style="width: 50%;">
                        <strong>SM of</strong> {{ $hpp->seniorManagerSignatureUser ? $hpp->seniorManagerSignatureUser->unit_work : 'N/A' }}
                    </td>
                </tr>
...
<tr>
    ...
    <td style="width:33%; padding:6px; font-size:10px; text-align:right; vertical-align:top; color:#333;">
        ...
        @endif
                ...
                <tr>
                    <td class="px-2 py-2 text-center" style="width: 50%; border-right: 1px solid black; border-bottom: 1px solid black;">
                        <strong>{{ $hpp->generalManagerSignatureUser ? $hpp->generalManagerSignatureUser->name : 'N/A' }}</strong>
                    </td>
                    <td class="px-2 py-2 text-center" style="width: 50%; border-bottom: 1px solid black;">
