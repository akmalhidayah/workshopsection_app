        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                ...
                <div class="px-4 py-5 sm:px-6 flex items-center justify-between">
                    <!-- Logo Kiri -->
                    <img src="{{ asset('images/logo-sig.png') }}" alt="Logo SIG" class="h-20 w-auto">
                    ...
                    <!-- Logo Kanan -->
                    <img src="{{ asset('images/logo-st.png') }}" alt="Logo Semen Tonasa" class="h-24 w-24">
                </div>
                    ...
                    <table class="min-w-full border-collapse">
                        ...
                        <tbody>
                            @foreach($scopeOfWork->scope_pekerjaan as $index => $pekerjaan)
                                <tr>
                                    <td class="border-2 border-black px-4 py-2 text-center">{{ $index + 1 }}</td>
                                    <td class="border-2 border-black px-4 py-2">{{ $pekerjaan }}</td>
                                    <td class="border-2 border-black px-4 py-2 text-center">{{ $scopeOfWork->qty[$index] ?? '-' }}</td>
                                    <td class="border-2 border-black px-4 py-2 text-center">{{ $scopeOfWork->satuan[$index] ?? '-' }}</td>
                ...
                <div class="p-6 grid grid-cols-3 gap-4">
                    ...
                    <div class="text-center">
                        ...
                        @if($scopeOfWork->tanda_tangan)
                            <img src="{{ $scopeOfWork->tanda_tangan }}" 
                                 alt="Tanda Tangan" 
                                 class="mt-2 mx-auto"
                                 style="max-height: 100px;">
                        @endif
                        <p class="mt-6 font-semibold">{{ $scopeOfWork->nama_penginput }}</p>
