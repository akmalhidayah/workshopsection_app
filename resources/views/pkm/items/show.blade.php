        ...
        <div class="max-w-5xl mx-auto bg-white rounded-lg p-8">
            ...
            <div class="mb-6 pb-4 border-b border-gray-200">
                ...
                <div class="grid grid-cols-2 gap-4 text-gray-900">
                    <p><strong class="text-orange-600">Nomor Notification:</strong> {{ $item->notification_number }}</p>
                    ...
                    <table class="min-w-full bg-white border border-gray-200 shadow-sm rounded-lg">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @php
                                ...
                                $harga = is_array($item->harga) ? $item->harga : json_decode($item->harga, true);
                            @endphp

                            @foreach ($materials as $index => $material)
                                <tr>
                                    <td class="px-4 py-3 border border-gray-300 text-center">{{ $index + 1 }}</td>
                                    <td class="px-4 py-3 border border-gray-300">{{ $material }}</td>
                                    <td class="px-4 py-3 border border-gray-300 text-right">
                                        Rp {{ number_format($harga[$index] ?? 0, 0, ',', '.') }}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
            ...
            <div class="grid grid-cols-3 gap-4 text-lg font-semibold text-center">
                <div class="px-6 py-3 rounded-lg">
                    ...
                    <p class="text-2xl text-blue-600">Rp {{ number_format($item->total, 0, ',', '.') }}</p>
                </div>
                <div class="px-6 py-3 rounded-lg">
                ...
                <div class="px-6 py-3 rounded-lg">
                    ...
                    <p class="text-2xl text-purple-600">
                        @if($item->total > 0)
                            {{ number_format(($item->total_margin / $item->total) * 100, 2) }}%
                        @else
                            0%
                        @endif
                    </p>
