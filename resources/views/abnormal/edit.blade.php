<x-app-layout>
    ...
    <div class="py-12">
        ...
        <div class="bg-white dark:bg-gray-900 overflow-hidden shadow-xl sm:rounded-lg p-6">
                <form method="POST" action="{{ route('abnormal.update', $abnormal->notification_number) }}" enctype="multipart/form-data">
                    ...
                    <div class="mt-6">
                        ...
                        <div id="action-container" class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                            @foreach ($abnormal->actions as $key => $action)
                                <div>
                                    <label class="block font-medium text-sm text-gray-800 dark:text-gray-300" for="action_{{ $key + 1 }}">
                                        Action {{ $key + 1 }}
                                    </label>
                                ...
                                <div>
                                    <label class="block font-medium text-gray-800 dark:text-gray-300" for="by_{{ $key + 1 }}">
                                        By
                                    </label>
                                ...
                                <div>
                                    <label class="block font-medium text-sm text-gray-800 dark:text-gray-300" for="when_{{ $key + 1 }}">
                                        When
                                    </label>
                    ...
                    <div class="mt-6">
                        ...
                        <div id="risk-container" class="grid grid-cols-1 gap-4 mt-2">
                            @foreach ($abnormal->risks as $key => $risk)
                                <div>
                                    <label class="block font-medium text-sm text-gray-800 dark:text-gray-300" for="risk_{{ $key + 1 }}">
                                        Risk {{ $key + 1 }}
                                    </label>
                    ...
                    <div class="mt-6">
                        <h3 class="font-semibold text-lg text-gray-800 dark:text-gray-300">Dokumentasi Abnormalitas</h3>
                        <div id="dokumentasi-container" class="grid grid-cols-1 gap-4 mt-2">
                            @foreach ($abnormal->files as $key => $file)
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block font-medium text-sm text-gray-800 dark:text-gray-300" for="foto_{{ $key + 1 }}">
                                    ...
                                    <div>
                                        <label class="block font-medium text-sm text-gray-800 dark:text-gray-300" for="keterangan_{{ $key + 1 }}">
    ...
    <script>
        document.getElementById('add-action').addEventListener('click', function () {
            ...
            const actionGroup = `
                <div>
                    <label class="block font-medium text-sm ttext-gray-800 dark:text-gray-300" for="action_${actionCount}">
                        Action ${actionCount}
                    </label>
